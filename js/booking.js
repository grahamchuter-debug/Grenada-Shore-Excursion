/**
 * Grenada request-to-book form (Phase 16D).
 * Server-side pricing is authoritative — client total is for review only.
 * Guest model: paying guests (ages 3+) only. Ages 0–2 not permitted.
 */
(function () {
  const form = document.getElementById("grd-booking-form");
  if (!form || !window.GRD_COMMERCIAL) return;

  const cfg = window.GRD_COMMERCIAL;
  ...
  const under16El = document.getElementById("under16_ack");
  ...
  if (live) {
    ...
    if (under16El) under16El.disabled = false;
    ...
    if (under16El) under16El.disabled = true;
  ...
  function sessionId() {
    ...
    return "grd-" + Date.now() + "-" + Math.random().toString(36).slice(2, 10);
  }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const adults = num("adults", 1);
    if (adults < 1) {
      showError("Please include at least one paying guest (ages 3+) as the booking lead.");
      return;
    }
    if (adults > max) {
    ...
    const under16Box = document.getElementById("under16_ack");

    if (!cruiseDate || !shipName || !name || !email || !phone) {
      showError("Please complete date, ship, and lead passenger details.");
      return;
    }
    if (!under16Box || !under16Box.checked) {
      showError("Please confirm age and adult-accompaniment rules for your party.");
      return;
    }
    ...
    notes.push("Under-16 accompaniment acknowledged");
