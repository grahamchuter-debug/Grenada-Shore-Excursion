/**
 * Grenada request-to-book Worker (Grenada's Spice Route only).
 *
 ...
 */
import { grenadaBookingCore } from "../../../shared/destinations/grenada";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Grenada Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findGrenadaBookingProduct(body.productId);
  ...
  const reference = createBookingReference(grenadaBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(grenadaBookingCore);
