 ...
 */
/** Phase 16D: Graham-authorised live request-to-book unlock (Grenada's Spice Route). */
export const LIVE_PAYMENTS_CODE_ENABLED = true;

export const LIVE_UNLOCK_PHRASE = "GRENADA_LIVE_UNLOCK";
...
export function liveReadinessGaps(
  ...
  if (!(env.SITE_BASE_URL || "").includes("grenadashoreexcursion.com")) gaps.push("site_base_url");
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Grenada. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!bookingsAreEnabled(env)) {
    return {
      ...
      message: "Grenada online booking is switched off. No payment was taken.",
    };
  }
  if (!env.DB) {
    return {
  ...
  if (!site.includes("grenadashoreexcursion.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on grenadashoreexcursion.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
